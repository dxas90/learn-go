...
check_deployment() {
    ...
    if ! kubectl get deployment learn-go >/dev/null 2>&1; then
        log_error "learn-go deployment not found"
        return 1
    fi

    # Check if deployment is available
    if kubectl get deployment learn-go -o jsonpath='{.status.conditions[?(@.type=="Available")].status}' | grep -q "True"; then
...
check_service() {
    ...
    if ! kubectl get service learn-go >/dev/null 2>&1; then
        log_error "learn-go service not found"
        return 1
    fi

    local cluster_ip=$(kubectl get service learn-go -o jsonpath='{.spec.clusterIP}')
...
quick_endpoint_test() {
    ...
    local service_ip=$(kubectl get service learn-go -o jsonpath='{.spec.clusterIP}')
    local service_port=$(kubectl get service learn-go -o jsonpath='{.spec.ports[0].port}')
